...
First, with the list endpoint (GET /list) they obtain the ``entity_id`` of all subordinates of the Trust Anchor. To obtain only the Wallet Provider ``entity_id``, a query parameter ``entity_type`` MAY be used. 
