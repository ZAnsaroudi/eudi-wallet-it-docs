...
Il Sistema IT-Wallet è un ecosistema federato che consente la gestione sicura dell'Identità Digitale e lo scambio di Credenziali Digitali per cittadini e organizzazioni. 
...
Il livello operativo primario implementa il ciclo di vita principale delle Credenziali Digitali attraverso interazioni coordinate tra Soluzioni Tecniche specializzate, in modo che tutte le operazioni delle credenziali mantengano gli standard di sicurezza e privacy consentendo al contempo esperienze utente fluide.
...
L'architettura abilita i seguenti processi di interazione principali:
  ...
  2. **Scoperta delle Credenziali e Gestione del Catalogo**: L'ecosistema supporta meccanismi duali di scoperta delle credenziali - scoperta pubblica attraverso il :ref:`registry:Catalogo degli Attestati Elettronici` per credenziali di uso generale, e scoperta privata tramite Credential Offer dirette per casi d'uso specifici. Il catalogo viene popolato automaticamente basandosi sulle policy dell'Organismo di Supervisione dalle definizioni registrate nel Claims Registry e nella Taxonomy.

  3. **Rilascio delle Credenziali**: I Credential Issuer si coordinano con le Fonti Autentiche tramite API standardizzate per richiedere Attributi dell'Utente verificati, creando Credenziali Digitali basate su dati autorevoli. Il rilascio può procedere sia per credenziali pubblicate nel catalogo che per offerte di credenziali private.

  4. **Archiviazione e Gestione delle Credenziali**: Le Soluzioni IT-Wallet ricevono e gestiscono le Credenziali Digitali sui dispositivi degli utenti, consentendo una gestione sicura del ciclo di vita delle credenziali.

  5. **Presentazione e Verifica delle Credenziali**: Gli Utenti presentano le Credenziali Digitali alle Relying Party, che verificano i claim attraverso validazione crittografica e verifica dello stato. Le Relying Party operano all'interno di ambiti autorizzati che definiscono quali tipi di credenziali e scopi possono richiedere, con validazione contro il :ref:`registry:Registro degli Attributi` per la verifica dello schema.
