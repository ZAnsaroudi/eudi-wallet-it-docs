...
.. list-table:: Campi di primo livello di ogni voce di Attestato Elettronico
  ...
  * - **authentic_sources**
    - OBBLIGATORIO. Array di informazioni rilevanti sulle Fonti Autentiche autorizzate, inclusi dati amministrativi e tecnici relativi alla fornitura di dati agli Emittenti di Credenziali. L'array DEVE includere almeno le seguenti informazioni:
      ...
      * **user_information**: [CONDIZIONALE] URI che punta a una pagina web contenente informazioni leggibili dall'utente sull'Attributo Elettronico, i cui dati sono forniti dalla Fonte Autentica. Questa pagina web DEVE mostrare qualsiasi informazione rilevante all'utente prima del flusso di emissione. Queste informazioni DEVONO essere comunicate all'Issuer durante l'onboarding della Fonte Autentica dalla Fonte Autentica stessa; se tali informazioni vengono comunicate all'Issuer, quest'ultimo DEVE pubblicarle sulla pagina web. Ad esempio, se il database della Fonte Autentica contiene i dati richiesti per gli attributi della Credenziale Digitale solo se registrati *dopo* una data specifica, queste informazioni DEVONO essere fornite sulla pagina web.
  * - **formats**
    - OBBLIGATORIO. Array dei formati tecnici supportati per le Credenziali Digitali, che include:
        ...
        * **vct**: CONDIZIONALE. È OBBLIGATORIO solo se il ``format`` è ``dc+sd-jwt``. DEVE essere impostato come una stringa URI nel formato ``https://{dominio Trust Anchor}/{version}/{credential_type}`` (ad esempio, ``https://trust-registry.eid-wallet.example.it/1.0/mDL``).
...
.. list-table:: Campi di Primo Livello di Ogni Voce di Attestato Elettronico
   ...
   * - **formats**
     - OBBLIGATORIO. Array dei formati supportati per l'Attestazione del Wallet, che include:
       ...
       * **vct**: CONDIZIONALE. È presente solo se il ``format`` è ``dc+sd-jwt``. DEVE essere impostato come una stringa URI nel formato ``https://{dominio Trust Anchor}/{version}/{credential_type}`` (ad esempio, ``https://trust-registry.eid-wallet.example.it/1.0/WalletAttestation``).
